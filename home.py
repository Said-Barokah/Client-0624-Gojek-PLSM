import pandas as pd, plspm.config as cp
...
def app():
    ...
    if data_master is not None:
        ...
        with st.spinner('Tunggu inisialisasi Outer Model'):
            time.sleep(3)
            config = cp.Config(structure.path(), scaled=False)
        ...
        plspm_calc_boot = Plspm(data_master, config, bootstrap=True, bootstrap_iterations=iterasi)
        st.subheader('Direct effects for paths')
        data = pd.DataFrame(plspm_calc_boot.bootstrap().paths())
        st.line_chart(data)
        st.write(data)

        st.subheader('Total effects for paths')
        data = pd.DataFrame(plspm_calc_boot.bootstrap().total_effects())
        st.line_chart(data)
        st.write(data)
        ...
        data = pd.DataFrame(plspm_calc_boot.bootstrap().weights())
        st.line_chart(data)
        st.write(data)